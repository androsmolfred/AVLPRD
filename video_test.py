...
# --- 1. CONFIGURATION ---
VIDEO_PATH = "videoStreamTest.mp4" 
...
# --- 2. EXCEL SETUP ---
print("Setting up Video Excel Log File...")
            ...
            if crop.size > 0:
                ...
                if best_plate and best_prob > 0.25:
                    ...
                    if best_plate not in seen_plates:
                        ...
                        if (current_time - seen_plates[best_plate]).total_seconds() > COOLDOWN_SECONDS:
                            allow_logging = True

                    if allow_logging:
                        print(f"[DETECTED] {best_plate} | State: {detected_state}")
                        try:
                            wb = load_workbook(LOG_FILE)
                            ws = wb.active
                            ws.append([current_time.strftime("%Y-%m-%d %H:%M:%S"), best_plate, detected_state, "Nigeria", round(best_prob, 2)])
                            
                            last_row = ws.max_row
                            green_fill = PatternFill(start_color="C6EFCE", end_color="C6EFCE", fill_type="solid")
                            for col_idx in range(1, 6): ws.cell(row=last_row, column=col_idx).fill = green_fill
